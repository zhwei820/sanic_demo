...
from aiodemo import demo, atomic_demo,  db_connect, db_close

app = Sanic(__name__)


@app.route("/a")
async def test_atomic(request):
    ret = await atomic_demo()
    return response.json(ret)

@app.route("/")
async def test_demo(request):
    ret = await demo()
    return response.json(ret)
...
from view.product import pro_bp, pro_bp_v1

app.blueprint(pro_bp)
app.blueprint(pro_bp_v1)
